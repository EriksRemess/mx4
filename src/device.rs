...
pub fn req(dev: &HidDevice, idx: u8, feature: u8, function: u8, params: &[u8]) -> Result<Vec<u8>> {
    ...
    // HID++ 2.0 short request layout: report, device, feature index, function/software ID, params.
    // `write_packet` expands this template to a long report for Bluetooth.
    let mut pkt = [0u8; 7];
    pkt[0] = SHORT;
    pkt[1] = idx;
    pkt[2] = feature;
...
fn find_supported_device(api: &HidApi) -> Option<&DeviceInfo> {
    // Prefer direct Bluetooth when both transports are connected; it addresses the mouse itself and
    // avoids accidentally configuring a stale receiver pairing.
    api.device_list()
...
fn device_index(info: &DeviceInfo) -> Result<u8> {
    ...
    // The Bolt HID++ collection's interface number corresponds to the receiver device slot used by
    // this mouse. Bluetooth bypasses receiver slots and always uses index zero above.
...
mod tests {
    ...
    fn surfaces_hidpp20_error_reply() {
        let reply = [0x10, 0x07, 0xff, 0x0b, 0x21, 0x09, 0];
    ...
    fn ignores_error_reply_for_another_request() {
        let reply = [0x10, 0x07, 0xff, 0x0c, 0x31, 0x09, 0];
        assert!(matches!(
            classify_reply(&reply, 0x07, 0x0b, 0x02).unwrap(),
            ReplyMatch::Ignore
        ));
